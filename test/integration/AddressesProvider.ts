...
describe.skip("AddressesProvider", function () {
  ...
  it("ensures lending pool address", async () => {
    const { addressesProvider, lendingPool, lendingPoolCore } =
      await loadFixture(deployContractFixtures);
    expect(await addressesProvider.getLendingPool()).to.equal(
      await lendingPool.getAddress(),
    );

    expect(await addressesProvider.getLendingPoolCore()).to.equal(
