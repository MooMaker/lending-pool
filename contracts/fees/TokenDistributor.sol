...
contract TokenDistributor is ReentrancyGuard, Initializable {
    ...
    /// @notice DEPRECATED
    address public constant KYBER_ETH_MOCK_ADDRESS =
    ...
    function distribute(IERC20[] memory _tokens) public {
        for (uint256 i = 0; i < _tokens.length; i++) {
            uint256 _balanceToDistribute = (address(_tokens[i]) !=
    ...
    function distributeWithPercentages(
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            uint256 _amountToDistribute = (address(_tokens[i]) !=
                EthAddressLib.ethAddress())
                ? _tokens[i].balanceOf(address(this)).mul(_percentages[i]).div(
                    100
                )
    ...
    function internalSetTokenDistribution(
        address[] memory _receivers,
        uint256[] memory _percentages
    ) internal {
        require(
    ...
    function internalDistributeTokenWithAmount(
        IERC20 _token,
        ...
        for (uint256 j = 0; j < _distribution.receivers.length; j++) {
            uint256 _amount = _amountToDistribute
                .mul(_distribution.percentages[j])
                .div(DISTRIBUTION_BASE);
            ...
            if (_tokenAddress != EthAddressLib.ethAddress()) {
                ...
                (bool _success, ) = _distribution.receivers[j].call{
                    value: _amount
                }("");
                require(_success, "Reverted ETH transfer");
            }
            emit Distributed(
                _distribution.receivers[j],
                _distribution.percentages[j],
                _amount
            );
        }
    }
